...
const features = [
  {
    ...
    desc: "Разместите объявление за 2 минуты — заполните форму, добавьте фото и готово",
    color: "from-amber-500 to-orange-500",
  },
  {
    icon: "Shield",
    title: "Безопасные сделки",
    desc: "Верификация пользователей, отзывы и рейтинги для вашей защиты",
    color: "from-emerald-500 to-teal-500",
  },
  {
    icon: "Search",
    title: "Умный поиск",
    desc: "Фильтры по цене, категории, городу и расстоянию до продавца",
    color: "from-violet-500 to-purple-600",
  },
  {
    icon: "MessageCircle",
    title: "Встроенный чат",
    desc: "Общайтесь прямо на сайте — никаких сторонних мессенджеров",
    color: "from-cyan-500 to-blue-500",
  },
  {
    icon: "Bell",
    title: "Уведомления",
    desc: "Получайте сообщения о новых объявлениях по вашим запросам",
    color: "from-pink-500 to-rose-500",
  },
  {
    icon: "TrendingUp",
    title: "Продвижение",
    desc: "Поднимите объявление в топ для максимального охвата",
    ...
    <div className="space-y-16">
      ...
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-violet-600 via-purple-700 to-cyan-600 p-8 sm:p-12 text-white">
        ...
        <div className="relative">
          ...
          <p className="text-white/90 text-lg max-w-2xl leading-relaxed">
            ...
            чтобы каждый человек в России мог с лёгкостью избавиться от
            ненужного или найти то, что давно искал — в своём городе или по всей
            стране.
          </p>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {features.map((f, i) => (
            <div
              key={f.title}
              className={`glass-card hover-scale rounded-2xl p-6 animate-fade-in delay-${(i + 1) * 100}`}
            >
              <div
                className={`w-12 h-12 rounded-xl bg-gradient-to-br ${f.color} flex items-center justify-center shadow-md mb-4`}
              >
      ...
      <section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-5">
          {team.map((member, i) => (
            <div
              key={member.name}
              className={`glass-card hover-scale rounded-2xl p-5 text-center animate-fade-in delay-${(i + 1) * 100}`}
            >
              <div
                className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${member.color} flex items-center justify-center mx-auto mb-3 shadow-md`}
              >
      ...
      <section className="glass-card rounded-2xl p-8 text-center">
        ...
        <p className="text-muted-foreground mb-6">
          Мы всегда рады помочь — напишите нам
        </p>
        <div className="flex flex-wrap justify-center gap-4">
          {[
            ...
            <div
              key={contact.label}
              className="flex items-center gap-2 px-4 py-2.5 bg-muted/60 rounded-xl text-sm font-medium"
            >
